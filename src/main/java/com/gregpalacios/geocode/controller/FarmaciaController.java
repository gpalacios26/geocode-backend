...
public class FarmaciaController {
	...
	@GetMapping
	public ResponseEntity<List<Farmacia>> listar() throws Exception {
		List<Farmacia> lista = service.listar();
		return new ResponseEntity<List<Farmacia>>(lista, HttpStatus.OK);
	}

	@GetMapping("/{id}")
	public ResponseEntity<Farmacia> listarPorId(@PathVariable("id") Integer id) throws Exception {
		Farmacia obj = service.listarPorId(id);

		if (obj == null) {
			throw new ModeloNotFoundException("ID NO ENCONTRADO " + id);
		}

		return new ResponseEntity<Farmacia>(obj, HttpStatus.OK);
	}
	...
	public @ResponseBody Map<String, Object> cargar() throws Exception {
		...
		if(lista.isEmpty()) {
			String fileName = "data/pharma-db.json";
			
			try {
	            ObjectMapper mapper = new ObjectMapper();
				TypeReference<List<Farmacia>> typeReference = new TypeReference<List<Farmacia>>(){};
				
				ClassLoader classLoader = getClass().getClassLoader();
		        InputStream inputStream = classLoader.getResourceAsStream(fileName);
		        
				List<Farmacia> farmacias = mapper.readValue(inputStream,typeReference);
				
				service.registrarTransaccional(farmacias);
